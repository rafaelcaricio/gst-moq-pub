...
/// MoQ video publisher example that publishes from a file source
#[derive(Parser, Debug)]
...
struct Args {
    ...
    /// Skip audio from source file
    #[arg(long)]
    no_audio: bool,
}

fn create_video_encoder() -> Result<(gst::Element, gst::Element), Box<dyn std::error::Error>> {
    ...
    Ok((videoenc, h264_capsfilter))
}

fn create_audio_encoder() -> Result<(gst::Element, gst::Element), Box<dyn std::error::Error>> {
    ...
    Ok((audioenc, audio_capsfilter))
}

fn setup_source_pipeline(
    ...
    // Create converters and encoders, but don't link them yet
    let videoconvert = gst::ElementFactory::make("videoconvert").build()?;
    let videorate = gst::ElementFactory::make("videorate").build()?;
    let (videoenc, h264_capsfilter) = create_video_encoder()?;
    pipeline.add_many([&videoconvert, &videorate, &videoenc, &h264_capsfilter])?;
    gst::Element::link_many(&[&videoconvert, &videorate, &videoenc, &h264_capsfilter])?;

    // Setup video pad on moqpub
    ...
    let video_src_pad = h264_capsfilter.static_pad("src").unwrap();
    video_src_pad.link(&video_pad)?;

    // Create audio elements if audio is enabled
    let audio_elements: Option<(gst::Element, gst::Element, gst::Element, gst::Element)> = if !no_audio {
        let audioconvert = gst::ElementFactory::make("audioconvert").build()?;
        let audioresample = gst::ElementFactory::make("audioresample").build()?;
        let (audioenc, audio_capsfilter) = create_audio_encoder()?;

        pipeline.add_many([&audioconvert, &audioresample, &audioenc, &audio_capsfilter])?;
        ...
        Some((audioconvert, audioresample, audioenc, audio_capsfilter))
    } else {
        None
    };
    ...
    decodebin.connect_pad_added(move |dbin, src_pad| {
        println!("Pad added on decodebin");
        ...
        match media_type.as_str() {
            "video/x-raw" => {
                ...
                if src_pad.link(&sink_pad).is_err() {
                    eprintln!("Failed to link video pad");
                }
            }
            "audio/x-raw" => {
                if !no_audio {
                    if let Some((audioconvert, _, _, _)) = &audio_elements
                    {
                        let sink_pad = audioconvert.static_pad("sink").unwrap();
                        if src_pad.link(&sink_pad).is_err() {
                            eprintln!("Failed to link audio pad");
                        }
                    }
                }
            }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Setup the source pipeline
    setup_source_pipeline(&pipeline, &moqpub, &args.source, args.no_audio)?;
